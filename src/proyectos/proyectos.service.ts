...
export class ProyectosService {
    ...
    async createProject ( createProyectoDto: CreateProyectoDto ) {
        ...
        const usuario = await this.usuario.findOneBy( { documento: usuarioDocumento } );

        if ( !usuario )
            throw new BadRequestException( `No existe o se encuentra desactivado el usuario con el documento ${ usuarioDocumento }` );

        try {
            const proyecto = await this.proyectoRepo.save( createProyectoDto );

            const { proyecto: { idProyecto }, usuario: { documento }, ...rest } = await this.usuariosProyectos.save( {
                proyecto, usuario,
